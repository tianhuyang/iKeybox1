...
NSString * CMD5::MDString (NSString *in)
{
    NSString *ret=nil;
    if (!in||in.length==0) {
        return ret;
    ...
    memset(out, 0, 17);
    MDString((uchar*)in.UTF8String,out,strlen(in.UTF8String));
    CharStr2HexStr(out,digest,16);
    ret=[[NSString alloc] initWithUTF8String:digest];
    return ret;
}
...
//    AnsiString s;
    ...
    //unsigned int len = lstrlenA ((char *)string);
